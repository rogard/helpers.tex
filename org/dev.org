...
#+begin_src latex
  ...
  %   and represents prenote, postnote, and key.
#+end_src
    ...
    {%
      ...
      {%
        ...
        \msg_warning:nnnn{{__erw_biblatex}{generic}
          { \tl_to_str:N\blx@bbxfile~\c__erw_biblatex_bbxfile_tl~inconsistent~with~template~`#1`}
          {Use~ \tl_to_str:N\blx@bbxfile~`numeric`~or~`numeric-comp`}}
      }
    }{}
    % 
